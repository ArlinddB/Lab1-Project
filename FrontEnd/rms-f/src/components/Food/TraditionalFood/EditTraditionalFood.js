...
function EditTraditionalFood(props) {
  ...
  useEffect( () => {
    axios.get(
      `https://localhost:5001/api/TraditonalFood/${ajdi}`, config)
      .then((response) => {
  ...
  const editFood = async () => {
    try {
      const response = await axios.put(
        `https://localhost:5001/api/TraditonalFood/${ajdi}`, data, config
      );
    } catch (error) {
...
export default EditTraditionalFood;
