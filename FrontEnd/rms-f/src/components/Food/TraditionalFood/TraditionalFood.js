...
import AddTraditionalFood from "./AddTraditonalFood";
import EditTraditionalFood from "./EditTraditionalFood";
...
const TraditionalFood = () => {
  ...
  useEffect(() => {
    axios.get("https://localhost:5001/api/TraditonalFood/", config)
    .then((res) => {
      setItems(res.data);
  ...
  const deleteFood = (foodid) => {
    if (window.confirm("Are you sure?")) {
      axios
        .delete("https://localhost:5001/api/TraditonalFood/" + foodid, config)
              ...
              <tbody>
                {paginatedItems.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <ButtonToolbar>
                        <EditTraditionalFood ajdi={item.foodId} />
