...
const FoodCategories = () => {
  ...
  useEffect(() => {
    axios.get("https://localhost:5001/api/foodcategory/", config)
    .then((res) => {
      setItems(res.data);
  ...
  const deleteCategory = (categoryId) => {
    if (window.confirm("Are you sure?")) {
      axios
        .delete("https://localhost:5001/api/foodcategory/" + categoryId, config)
              ...
              <tbody>
                {paginatedItems.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <ButtonToolbar>
                        <EditFoodCategories ajdi={item.categoryId} />
