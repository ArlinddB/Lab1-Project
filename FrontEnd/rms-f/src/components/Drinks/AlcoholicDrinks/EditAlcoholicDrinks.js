...
function EditAlcoholicDrinks(props) {
  ...
  useEffect( () => {
    axios.get(
      `https://localhost:5001/api/AlcoholicDrinks/${ajdi}`, config)
      .then((response) => {
  ...
  const editDrink = async () => {
    try {
      const response = await axios.put(
        `https://localhost:5001/api/AlcoholicDrinks/${ajdi}`, data, config
      );
    } catch (error) {
...
export default EditAlcoholicDrinks;
