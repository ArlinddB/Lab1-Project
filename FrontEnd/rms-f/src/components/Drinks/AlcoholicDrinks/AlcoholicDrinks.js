...
import AddAlcoholicDrinks from "./AddAlcoholicDrinks";
import EditAlcoholicDrinks from "./EditAlcoholicDrinks";
...
const AlcoholicDrinks = () => {
  ...
  useEffect(() => {
    axios.get("https://localhost:5001/api/AlcoholicDrinks/", config)
    .then((res) => {
      setItems(res.data);
  ...
  const deleteDrink = (drinkid) => {
    if (window.confirm("Are you sure?")) {
      axios
        .delete("https://localhost:5001/api/AlcoholicDrinks/" + drinkid, config)
              ...
              <tbody>
                {paginatedItems.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <ButtonToolbar>
                        <EditAlcoholicDrinks ajdi={item.drinkId} />
