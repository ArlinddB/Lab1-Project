...
function EditHotDrinks(props) {
  ...
  useEffect( () => {
    axios.get(
      `https://localhost:5001/api/HotDrinks/${ajdi}`, config)
      .then((response) => {
  ...
  const editDrink = async () => {
    try {
      const response = await axios.put(
        `https://localhost:5001/api/HotDrinks/${ajdi}`, data, config
      );
    } catch (error) {
