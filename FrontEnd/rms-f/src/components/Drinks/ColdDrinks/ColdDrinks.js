...
import AddColdDrinks from "./AddColdDrinks";
import EditColdDrinks from "./EditColdDrinks";
...
const ColdDrinks = () => {
  ...
  useEffect(() => {
    axios.get("https://localhost:5001/api/ColdDrinks/", config)
    .then((res) => {
      setItems(res.data);
  ...
  const deleteDrink = (drinkid) => {
    if (window.confirm("Are you sure?")) {
      axios
        .delete("https://localhost:5001/api/ColdDrinks/" + drinkid, config)
              ...
              <tbody>
                {paginatedItems.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <ButtonToolbar>
                        <EditColdDrinks ajdi={item.drinkId} />
